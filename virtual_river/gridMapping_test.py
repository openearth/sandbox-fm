...
def hex_to_points(hexagons, grid, changed_hex=None, start=False, turn=0):
    ...
    counter = 0
    for feature in grid.features:
        if feature.properties["board"] or feature.properties["fill"] or feature.properties["border"]:
            continue
        nearest = feature.properties["nearest"]
    ...
    filename = 'grid_with_z_triangulate_test_%d.geojson' % turn
...
def create_geotiff(grid):
    ...
    for feature in grid.features:
        ...
        print(new_feature.properties['z'])
    ...
    a = rasterize(zip(geometries, out), out_shape=(0, 400))
    plt.imshow(a)
    """
    for feature in grid.features:
        if feature.properties["board"]:
